...
"""
Fact checker for fake news detection
Main inference module for the neuro-symbolic approach
"""
...
try:
    from .modules.data_processor_db import DataProcessor
...
class FactChecker:
    """Main fact checking class for fake news detection"""
    ...
    def _prepare_ckpt(self, model_name_or_path, ckpt_dir):
        """Prepare checkpoint directory with tokenizer"""
    ...
    def _check(self, inputs: list, batch_size=32, verbose=True):
        """Perform fact checking on input data"""
    ...
    def check_from_file(self, in_filename, out_filename, batch_size, verbose=False):
        """Check facts from input file and save results"""
    ...
    def check_from_batch(self, inputs: list, verbose=False):
        """Check facts from batch of inputs"""
