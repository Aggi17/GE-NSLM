...
class BertChecker(BertPreTrainedModel):
    def __init__(self, config, logic_lambda=0.0, prior='nli', m=8, temperature=1):
        ...
        self.bert = BertModel(config)   # 应该参数不更新吧 对啊，直接加载的预训练模型
    ...
    def forward(self, claim_input_ids, claim_attention_mask, claim_token_type_ids,
        ...
        # unse(1),在第2维上增加维度，eg,(3,4) 变成 (3,1,4), global 扩维复制后也变成了b x m x h, 和local再在最后一维上拼接，b x m x 2h
        ...
        if labels is not None:
            ...
            y_star_emb = get_label_embeddings(labels_onehot, self.classifier.out_proj.weight)  # b x h   这里可能是对label转换，编码成向量的形式
            ...
            # ======================== ELBO =================================
            elbo_neg_p_log = F.cross_entropy(y.view(-1, self.num_labels), labels.view(-1))  # y是P_theta的输出，相当于重建的数据  labels 应该就是真实标签啊
            # view中一个参数定为-1，代表自动调整这个维度上的元素个数，以保证元素的总数不变。一维的可能就变成n维的
            if self.prior == 'nli':
            ...
            # 生成一个KL散度损失函数，常用于衡量两个连续分布的距离
            # 由于KL散度具有不对称性，存在一个指导和被指导的关系，如果想用 y去指导 x，则第一个参数需要传 x，第二个参数需要传 y

            neg_elbo = elbo_kl + elbo_neg_p_log   # （3）式

            loss = (1 - abs(self._lambda)) * neg_elbo + abs(self._lambda) * logic_loss  # （5）式
        else:
            # Inference   测试 先验不会有伪标签那一项
            if self.prior == 'nli':
                z = nli_labels
            elif self.prior == 'uniform':
                prior = torch.tensor([1 / self.num_labels] * self.num_labels).to(y) # 都是1/3
    ...
    def P_theta(self, X_global, X_local, z):
        ...
        :param X_local: b x m x h  不对吧 应该是 b x h
        :param z: b x m x 3
        :param mask: b x m
        :return: b x 3, b x m
        '''
